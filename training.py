...
path_results = os.path.join('results', 'trainNN11')
...
# training parameters
num_iter = 1001 # amount of synthetic surface samples during training
                    ...
                    def _forward(network: nn.Module, data: DataLoader, metric: callable):
                        ...
                        for synthetic_surface, idx in data:
                            ...
                            if encoder_decoder:
                                ...
                                res = _sigma * metric(predicted_surface[..., crop:-crop, crop:-crop],
                                ...
                                res += (1-_sigma) * metric(predicted_gradients, synthetic_gradients)
                            yield res
                    ...
                    def update(network: nn.Module, data: DataLoader, loss: nn.Module,
                        ...
                        for iter, err in tqdm(enumerate(_forward(network, data, loss))):
                            ...
                            if (iter % 100) == 0 and iter != 0:
                                ...
                                synthetic_surface = dataset.__getitem__(0)[0].unsqueeze(0)
                                synthetic_images = transformation.forward(synthetic_surface.to(device))
                                predicted_surface = network(synthetic_images)
                                res = loss(predicted_surface[..., crop:-crop, crop:-crop],
                                                      (synthetic_surface[..., crop:-crop, crop:-crop]).to(device))
                                ...
                                plt.subplot(1, 2, 1)
                                plt.imshow(network.gradients.cpu().numpy()[0,0,crop:-crop, crop:-crop])
                                plt.title('surface-loss')

                                plt.subplot(1, 2, 2)
                                plt.imshow(synthetic_images.cpu().numpy()[0,0, crop:-crop, crop:-crop])
                                plt.title('input')

                                plt.savefig(os.path.join(path, f'test-surface-loss{iter}.png'))
                                plt.close()
                                ...
                                predicted_gradients = getGradients(predicted_surface[..., crop:-crop, crop:-crop])
                                synthetic_gradients = getGradients((synthetic_surface[..., crop:-crop, crop:-crop]).to(device))
                                res = loss(predicted_gradients, synthetic_gradients)
                                ...
                                plt.savefig(os.path.join(path, f'test-gradients-loss{iter}.png'))
                                plt.close()
                                ...
                                predicted_images = transformation.forward(predicted_surface)
                                # calculate loss between synthetic images and predicted images
                                res = loss(predicted_images[..., crop:-crop, crop:-crop],
                                             synthetic_images[..., crop:-crop, crop:-crop])
                                ...
                                plt.savefig(os.path.join(path, f'test-image-loss{iter}.png'))
                                plt.close()
